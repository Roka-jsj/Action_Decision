...
DST=packages/submit_m1swapcal
MEMBER=work/member_m1h8full_q8dir
...
rm -rf "$DST" packages/submit_m1swapcal.zip
cp -al "$SRC" "$DST"
...
(cd "$DST" && zip -qr "$R/packages/submit_m1swapcal.zip" .)
ls -la packages/submit_m1swapcal.zip
python3 sim/check_zip.py packages/submit_m1swapcal.zip
