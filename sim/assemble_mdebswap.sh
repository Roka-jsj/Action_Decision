#!/bin/bash
# mdebswap 조립 — th85 클론 + m2 '가중치 파일만' 우리 mdebfull(fp16)로 교체. (R68)
...
DST=packages/submit_mdebswap
MEMBER=work/member_mdebfull
...
rm -rf "$DST" packages/submit_mdebswap.zip
cp -al "$SRC" "$DST"
...
DST = f"{R}/packages/submit_mdebswap"
...
old_fs = set(os.listdir(f"{SRC}/model/m2"))
new_fs = set(os.listdir(f"{DST}/model/m2"))
missing = old_fs - new_fs
assert not missing, f"우리 멤버에 없는 파일: {missing}"
assert "model.safetensors" in new_fs and "qweights.npz" not in new_fs
c_old = json.load(open(f"{SRC}/model/m2/config.json"))
c_new = json.load(open(f"{DST}/model/m2/config.json"))
...
for f in new_fs:
    ...
    if os.path.exists(old):
        assert os.stat(old).st_ino != os.stat(f"{DST}/model/m2/{f}").st_ino, f"m2/{f} hardlink 공유"
# 가중치가 실제로 다른지(스왑 실효성) + 토크나이저 파일 지문 기록
w_old, w_new = sha16(f"{SRC}/model/m2/model.safetensors"), sha16(f"{DST}/model/m2/model.safetensors")
assert w_old != w_new, "가중치 동일 — 스왑 무의미"
print(f"[mdebswap] 구조증명 OK — m2 가중치 {w_old}→{w_new}, 그 외 전부 inode 불변")
PY
...
echo "[mdebswap] 앵커 불변 증명 OK"

(cd "$DST" && zip -qr "$R/packages/submit_mdebswap.zip" .)
ls -la packages/submit_mdebswap.zip
python3 sim/check_zip.py packages/submit_mdebswap.zip
