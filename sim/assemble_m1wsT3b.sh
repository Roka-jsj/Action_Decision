...
DST=packages/submit_m1wsT3b
MEMBER=work/member_m1wsT3b_q8dir
...
rm -rf "$DST" packages/submit_m1wsT3b.zip
cp -al "$SRC" "$DST"
...
(cd "$DST" && zip -qr "$R/packages/submit_m1wsT3b.zip" .)
ls -la packages/submit_m1wsT3b.zip
python3 sim/check_zip.py packages/submit_m1wsT3b.zip
