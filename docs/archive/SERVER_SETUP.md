...
> **철칙 2**: 모든 작업은 **`/root/Action_Decision`** 아래에서 — 컨테이너 진입(-w 지정) 즉시 이 디렉터리에서 시작하므로 cd 불필요.
...
```
**블록 2 (데이터·멤버 반입 + 부트스트랩)** — 인증 불필요, 직링크:
```bash
curl -L -o bundle.zip "https://www.kaggle.com/api/v1/datasets/download/tistmesp03/ad236694-train-bundle"
unzip -q bundle.zip -d bundle && rm bundle.zip
bash sim/server_bootstrap.sh ./bundle    # "bootstrap OK: 70000 samples" 확인
```
(정상이면 1.5GB. 수 KB짜리가 받아지면 데이터셋이 비공개로 바뀐 것 — kaggle.com에서 공개로 전환하거나 부록 A 인증 후 CLI로.)
(curl DNS 오류 시: `echo 'nameserver 8.8.8.8' >> /etc/resolv.conf` 후 재시도)

### Claude Code / codex (VS Code 확장)
로컬 VS Code: **Remote-SSH** 접속 → Remote Explorer → **Attach to Running Container → mun-jtrain** → 폴더 `/root/Action_Decision` 열기 → 확장(Claude Code, Codex) 설치·로그인. 이후 전략·실험 지휘는 이 Claude가 담당.

## 3. 전자동 실험 — 원커맨드 (학습→패키징→검증→결과)

```bash
mkdir -p work
nohup bash sim/train_and_verify.sh largev6s3 777 > work/auto_largev6s3.log 2>&1 &
tail -f work/auto_largev6s3.log    # 관전 (Ctrl+C로 관전만 중단, 작업은 계속)
```
`train_and_verify.sh <TAG> <SEED> [EPOCHS=8] [VERSION=v6] [TEST_CT=mun-jtest]` 가 자동 수행:
1. FULL 학습 (A6000 ~40분; member 있으면 스킵) → `action_decision_maximum/experiments/member_<TAG>.zip`
2. bias 포함 패키징 + zip 구조검사
3. `/share`에 자가완결 검증셋 조립 (holdout 30k + 무의존 채점기)
4. **검증 컨테이너를 docker exec로 자동 실행** (오프라인 30k 추론)
5. 결과 → `work/verify_<TAG>.txt`: A6000시간 / 피크VRAM / **환산 서버시간 게이트(≤540s)** / **VRAM 게이트(≤14GB)** / holdout macro-F1
...
echo '{"ratio": 3.21}' > sim/calib.json    # 이후 모든 검증에 자동 동봉
```
(tri_cond 앵커 427초로 교차확인 권장. 컨테이너 Claude에게 "캘리브레이션 해줘"로 위임 가능)

## 5. 산출물 백업·제출

- **원칙: 인증 필요한 작업은 최대한 배제.** 파일 이동은 전부 무인증 경로로:
  - 제출 zip 회수(서버→로컬): 호스트 터미널 `docker cp mun-jtrain:/root/Action_Decision/packages/submit_X.zip .` → 로컬로 scp/다운로드 → Dacon 웹 수동 제출 → 점수를 컨테이너 Claude에게 전달
  - member zip 백업: 같은 방식으로 `docker cp` → 로컬 보관 (컨테이너 소실 대비)
  - 코드: git push (컨테이너 안 VS Code의 GitHub 인증은 확장이 처리 — 1회)
- Colab/Kaggle GPU 은퇴 — Kaggle 데이터셋은 다운로드(직링크) 전용 채널.

## 부록 A. kaggle 인증 (선택 — 데이터셋에 새 멤버를 업로드할 때만)
kaggle.com→Settings→API→Create New Token의 key만 복사해 두고:
```bash
pip install -q kaggle && mkdir -p ~/.kaggle
read -p "Kaggle key 붙여넣기: " K && echo "{\"username\":\"tistmesp03\",\"key\":\"$K\"}" > ~/.kaggle/kaggle.json && chmod 600 ~/.kaggle/kaggle.json
cp action_decision_maximum/experiments/member_<TAG>.zip kaggle/ds/
KAGGLE_API_TOKEN=$(cat ~/.kaggle/kaggle.json) kaggle datasets version -p kaggle/ds -m "add <TAG>" --dir-mode zip
```
