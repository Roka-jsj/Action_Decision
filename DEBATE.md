...
- **LB 갱신(조원, 07-09)**: cc_tri_w2 **0.78554 신은행**(w2=0.20, +0.0003) / cc_tri_koel 0.78406(koelectra 편입 음수 — base급 한계 실증). **시간캡 재조정(사용자 지적)**: 실채점 최장 8:56(536s) 정상 통과 — 실캡 600s, 내부 540s 게이트는 보수였음. C0(추정 432s) 검증 생략 제출 승인(사용자).
...
- **다음 좌표(같은 밤, 쿼터 소멸 전)**: wm20 = 0.55/0.20/0.25(mdeb 0.15→0.20 단일변경) — 근거는 노이즈 기울기가 아니라 **조원 LB 실측 w2=0.20 +0.0003**(형제 구조 tri에서 mdeb 가중 동일 방향 양성). 사전등록: 중앙 +0.0002 [−0.0006, +0.0010], 채택문턱 +0.0005, 시간 ≈380s.
